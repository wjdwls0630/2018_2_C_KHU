...
void Application::Run() {
	...
	while ((command = GetCommand())) {
		...
		case  0:
			return;
		case 1:			  // 파일에서 모든 사진 읽기
			ReadAllFromFile();
			break;
		case 2:			  // 리스트의 모든 사진을 화면에 출력
			DisplayList();
			break;
		case 3:			  // 리스트 모든 사진을 파일에 출력
			WriteAllToFile();
			break;
		case 4:		// Read in a record and add to the list
			AddPhoto();
			break;
		case 5:			  // 사진명을 입력받아서 해당 사진을 리스트에서 삭제
			DeletePhoto();
			break;
		case 6:	  // 사진명을 입력받아서 리스트에서 검색.
			FindPhoto();
			break;
		case 7:	  // 이벤트 명을 입력받아서 리스트에서 검색.
			FindPhotoByEvent();
			break;
		case 8:
...
int Application::GetCommand()
{
	...
	while (1)
	{
		...
		cout << "\t|" << setw(50) << "    1. 파일에서 모든 사진 읽기" << "|\n";
		cout << "\t|" << setw(50) << "    2. 리스트의 모든 사진을 화면에 출력" << "|\n";
		cout << "\t|" << setw(50) << "    3. 리스트의 모든 사진을 파일에 출력" << "|\n";
		cout << "\t|" << setw(50) << "    4. 새 사진 추가" << "|\n";
		cout << "\t|" << setw(50) << "    5. 사진 삭제" << "|\n";
		cout << "\t|" << setw(50) << "    6. 사진 이름으로 검색" << "|\n";
		cout << "\t|" << setw(50) << "    7. 이벤트 이름으로 검색" << "|\n";
		cout << "\t|" << setw(50) << "    8. 이벤트 리스트를 이용한 이벤트 검색" << "|\n";
		cout << "\t|" << setw(50) << "    9. 이벤명과 사진명으로 검색" << "|\n";
		cout << "\t|" << setw(50) << "    10.이벤명과 내용 키워드로 검색" << "|\n";
		cout << "\t----------------------------------------------------" << endl;
		cout << "\t 수행할 작업 선택 --> ";
